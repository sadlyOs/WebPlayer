...
class MusicRepository:
    ...
    async def create_playlist(self, user_id: int, title: str):
        playlist = Play_list(user_id=user_id, title=title)
    ...
    async def get_all_music(self):
        execute = await self.__session.execute(select(Music))
        music = execute.scalars().all()
        return music

    async def get_all_playlists(self):
        execute = await self.__session.execute(select(Play_list).options(selectinload(Play_list.music)))
